    ...
    : QMainWindow(parent)
{
    ...
    BufferPoints.clear();
    ui.setupUi(this);
    connect(ui.SourceButton, &QPushButton::clicked, this, &Cropping::SwitchToSource);
    connect(ui.WindowButton, &QPushButton::clicked, this, &Cropping::SwitchToWindow);
    connect(ui.SaveButton, &QPushButton::clicked, this, &Cropping::Weiler_Atherton);
}
...
//�ж��Ƿ�˳ʱ��
bool Cropping::IsClockWise(list<Point> PointLink)
{
    int sum = 0;
...
bool Cropping::JudgeInside(Point p, list<Point> Polygon)
{
    double sum = 0;
...
//�л�ģʽ
void Cropping::SwitchToSource()
{
    mode = SOURCE;
    BufferPoints.clear();
}
void Cropping::SwitchToWindow()
{
    mode = WINDOW;
    BufferPoints.clear();
}

Point Cropping::GetIntersect(Point p_start, Point p_end, Point q_start, Point q_end)
{
...
double Cropping::GetK(Point start, Point end, Point middle)
{
...
void Cropping::GetIntersections(Point start_a, Point end_a, int npolygon, int nline, vector<list<Point>>& Window,
...
void Cropping::SortIntersections(vector<Intersection>& t_list)
{
...
void Cropping::FindRelatedPlace(bool type, Intersection p, int& npolygon, int& nline, int& npoint)
{
    nline = p.lineNumber;
    npolygon = p.polygon_number;
    //��window��
    if (type == SOURCE)
    {
        for (int i = 0; i < list_window[npolygon][nline].size(); i++)
        {
            if (list_window[npolygon][nline][i].x_coordinate == p.x_coordinate && list_window[npolygon][nline][i].y_coordinate == p.y_coordinate)
            {
                npoint = i;
                return;
            }
    ...
    else
    {
        ...
        {
            if (list_source[npolygon][nline][i].x_coordinate == p.x_coordinate && list_source[npolygon][nline][i].y_coordinate == p.y_coordinate)
            {
                npoint = i;
                return;
            }
...
void Cropping::FindNextPlace(int& npolygon, int& nline, int& npoint, bool& pmode,
                             int cpolygon, int cline, int cpoint, bool cmode, int current)
{
    //cpolygon cline cpoint-1����ʼ��
...
void Cropping::Weiler_Atherton()
{
    ...
    while(1)
    {
        ...
        {
            Point nova = Point(list_source[v_polygon][v_line][v_point].x_coordinate, list_source[v_polygon][v_line][v_point].y_coordinate);
...
bool Cropping::HasNoIntersection(bool mode, int num)
{
    if (mode == SOURCE)
    {
...
bool Cropping::JudgeAdd(bool mode, int num)
{
    if (mode == SOURCE)
    {
        for (int i = 0; i < WindowPoints.size(); i++)
...
void Cropping::JudgeSpecial()
{
    //���У�û�������
    //1.Ŀ���⻷û���㣺�⻷�ڴ�����������棬���Ҳ����κδ����ڻ��ڣ��ͱ���
    //2.Ŀ���ڻ�û���㣺�ڻ��ڴ�����������棬���Ҳ����κδ����ڻ��ڣ��ͱ���
    //3.�����⻷û���㣺��Ŀ���⻷�ڣ����Ҳ����κ�Ŀ���ڻ��ڣ��ͱ���
    //4.�����ڻ�û���㣺��Ŀ���⻷�ڣ����Ҳ����κ�Ŀ���ڻ��ڣ��ͱ���
    for (int i = 0; i < SourcePoints.size(); i++)
    {
